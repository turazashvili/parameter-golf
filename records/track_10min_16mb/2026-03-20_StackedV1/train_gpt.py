...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if _HAS_FA3:
            # FA3 expects (bsz, seqlen, heads, head_dim). GQA handled natively.
            y = _fa3_func(
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        if self.tie_embeddings:
            ...
            # Overtone init: shape embedding spectrum to power-law decay (S_k ~ k^{-0.5}).
            with torch.no_grad():
...
def main() -> None:
    ...
    # SWA: collect checkpoint snapshots during warmdown for weight averaging.
    ...
    if master_process:
        ...
        log0(f"Total submission size int8+{compress_label}: {quant_file_bytes + code_bytes} bytes")
