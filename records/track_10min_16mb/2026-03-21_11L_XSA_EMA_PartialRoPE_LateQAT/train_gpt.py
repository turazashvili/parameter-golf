...
class Hyperparameters:
    ...
    ema_every = int(os.environ.get("EMA_EVERY", 4))
...
class CausalSelfAttention(nn.Module):
    ...
    def _xsa_remove_self(self, y: Tensor, v: Tensor) -> Tensor:
        """Remove self-value projection from attention output (XSA)."""
        B, T, H, D = y.shape
        Hkv = v.size(-2)
        group = H // Hkv
...
def main() -> None:
    ...
    # NCCL tuning for multi-GPU throughput (no effect on single GPU).
    if world_size > 1:
    ...
    swa_count = 0
    ema_params: list[Tensor] | None = None
    ema_names: list[str] | None = None
    if args.ema_enabled:
        sd = base_model.state_dict()
        ema_names = list(sd.keys())
        ema_params = [t.detach().float().clone() for t in sd.values()]
    ...
    while True:
        ...
        # Late QAT: enable STE fake-quantization in final ~4% of training.
        ...
        # EMA: exponential moving average using fast batched ops, every N steps.
        if ema_params is not None and step % args.ema_every == 0:
            with torch.no_grad():
                current = [t.detach().float() for t in base_model.state_dict().values()]
                torch._foreach_lerp_(ema_params, current, 1.0 - args.ema_decay)
    ...
    # Apply EMA or SWA weights before serialization.
    if ema_params is not None and ema_names is not None:
        log0("ema:applying EMA weights")
        sd = base_model.state_dict()
        avg_state = {name: ema_t.to(dtype=sd[name].dtype) for name, ema_t in zip(ema_names, ema_params)}
        del ema_params
